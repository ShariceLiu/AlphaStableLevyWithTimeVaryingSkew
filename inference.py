...
def particle_filter_1d(y_ns, P, c, T, sigma_mu, sigma_w, noise_sig, alpha,l, delta_t, trans_As = None, noise_Cs=None, alpha_w = 0.000000000001,beta_w = 0.000000000001):
    ...
    for n in tqdm(range(N-1)):
        ...
        for p in range(P):
            ...
            processer = alphaStableJumpsProcesser(gammas, vs, alpha, delta_t_n, l)
            
            # transition matrices
            C = noise_variance_C(processer.S_mu(sigma_mu),processer.S_s_t(sigma_w), processer.hi_fi_intQ(sigma_mu,l), processer.hi_fi_vi(sigma_mu),delta_t_n, l, sigma_mu)
            A = transition_matrix(processer.mean_s_t(), delta_t_n, l)
