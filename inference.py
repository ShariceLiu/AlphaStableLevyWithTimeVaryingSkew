...
def particle_filter_1d(y_ns, P, c, T, sigma_mu, sigma_w, noise_sig, alpha,alpha_w = 0.000000000001,beta_w = 0.000000000001):
    ...
    for n in range(N):
        ...
        if n%5 == 0: # resample
            n_log_ws[n-1], n_mus[n-1], n_vars[n-1], E_ns[n-1] = resample(n_log_ws[n-1], n_mus[n-1], n_vars[n-1], E_ns[n-1])
        ...
        for p in range(P):
            ...
            E_ns[n,p] = -(y_n-y_hat_n_prev_n)**2/(norm_sigma_n_prev_n)/2
            beta_w_post_p = beta_w - sum(E_ns[:,p])
            log_like = -0.5*np.log(sigma_n_prev_n)-(alpha_w+n*2/2)*np.log(beta_w_post_p)\
                    +((n-1)*2/2+alpha_w)*np.log(beta_w - sum(E_ns[:n,p]))+\
        ...
        # normalise weights
        n_log_ws[n,:] = n_log_ws[n,:]- np.log(sum(np.exp(n_log_ws[n,:])))
...
if __name__=='__main__':
    ...
    num_particles = 1
    n_mus, n_vars, n_log_ws, E_ns = particle_filter_1d(y_ns, num_particles, c, delta_t, sigma_mu, sigma_w, noise_sig = k_v*sigma_w, alpha=alpha)
