...
def inf_finance(num_particles = 200, data = 'nvdia', alpha = 0.8, l = -1, c=10, sigma_w = 2, sigma_mu = 1, k_v=1):
    ...
    with open(f'experiments/figure/simplified/{data}/marginals.txt', 'a') as f:
...
def marg_wrt_l(alpha = 0.9, label = 'simu_data', num_particles = 200, k =5,  save = True):
    ...
    if save:
        ...
        for j, ld in enumerate(lds):
            ...
            for i in range(k):
                ...
                # marg = inf_finance(num_particles=200, k_v=1e3, l=-ld, sigma_mu=1e-4, sigma_w=3.0e-4, c=5)
                margs[i] = marg
            means[j]=margs.mean()
    ...
    plt.xlabel(r'$\lambda$')
    plt.xticks(ticks= log_lds,labels=[round(l, 3) for l in lds])
    plt.ylabel('log marginal')
    plt.legend(['Marginals', r'True $\lambda$'])
    plt.savefig(f'experiments/figure/simplified/marg/marg_l_a{int(alpha*10)}.png')


if __name__=='__main__':
    alpha = 0.9
    marg_wrt_l(alpha=1.2, num_particles=100, k=5, save=False)

    # x = simu_2d(save= True, alpha= alpha, sigma_w=0.05, k_v=1e3) # 1e4 for 0.9
    ...
    # test_data_2d(num_particles=200, alpha=alpha)
    # plot_result_from_stored(alpha=alpha)
